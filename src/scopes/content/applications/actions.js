...
export function createEUIAsync(application: TTNApplication, eui: ?string) {
  const { id } = application
  return async (dispatch: Dispatch, getState: GetState) => {
    let payload
    try {
      if (eui) {
        payload = await apiClient.put(APPLICATIONS + id + `/euis/${eui}`)
      } else {
        // EUI Generated by backend
        payload = await apiClient.post(APPLICATIONS + id + '/euis')
      }
